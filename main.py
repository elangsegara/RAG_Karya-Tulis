...
st.set_page_config(page_title="Mesin Penjawab Query PDF", 
                   layout="wide", 
...
if upload_file:
    ...
    if "last_uploaded_file" not in st.session_state or st.session_state["last_uploaded_file"] != file_id:
        with st.spinner("Melakukan ekstraksi dan indeksasi PDF..."):
            ...
            if not chunks:
                st.error("Tidak ada teks yang berhasil diekstrak dari PDF. Pastikan file tidak berupa hasil scan gambar.")
                st.stop()
